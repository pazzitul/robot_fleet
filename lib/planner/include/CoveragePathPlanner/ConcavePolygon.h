...
class ConcavePolygon
{
    ...
    double mult(Vec2 a, Vec2 b, Vec2 c)
    {
...
//        return true;
        ...
        {
            tempSegment.startPos = polygonVertices[i].position;
            tempSegment.finalPos = polygonVertices[mod(i+1, polygonVertices.size())].position;
            if(hasIntersection(tempSegment, ls))
            {
                num++;
            }
        }
                               ...
                               Vec2 const & origin)
    {
        ...
        else if(indices.size() > 1)
        {
            ...
            for(unsigned int i=0; i<indices.size(); ++i)
            {
                ...
                bool a = Vertex::getHandedness(prevVert, currVert, nextVert) < 0.0f;
                bool b = checkVisibility(origin, polygonVertices[index], polygonVertices);
