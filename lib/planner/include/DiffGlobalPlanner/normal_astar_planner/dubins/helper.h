...
static inline float normalizeHeadingRad(float t) {
  if (t < 0) {
    ...
    return 2.f * M_PI + t;// > M_PI ? 2.f * M_PI + t - M_PI : 2.f * M_PI + t; //[-PI, PI]
  }

  return t - 2.f * M_PI * (int)(t / (2.f * M_PI));// > M_PI ? t - 2.f * M_PI * (int)(t / (2.f * M_PI)) - M_PI: t - 2.f * M_PI * (int)(t / (2.f * M_PI)); //[-PI, PI]
}
